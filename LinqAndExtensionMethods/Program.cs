...
namespace LinqAndExtensionMethods {
    class Program {
        static void Main(string[] args) {
            ...
            var ProductsWithPrices = from pro in productList
                                     join pri in priceList
                                     on pro.Code equals pri.ProductCode
                                     select new {
                                         Code = pro.Code,
                                         Name = pro.Name,
                                         Price = pri.Price,
                                         Quantity = pro.Quantity,
                                         Total = pro.Quantity * pri.Price
                                     };
            ...
            var nbrs2SumDivBy3 = nbrs2.Where(x => x % 3 == 0).Sum();
            Console.WriteLine($"Sum: {nbrs2SumDivBy3}");
            var nbrs2Avg = nbrs2.Average();
            Console.WriteLine($"Average: {nbrs2Avg}");
            var nbrsSumDivBy3Ext = from n in nbrs2 where n % 3 == 0 select n;
